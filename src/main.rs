...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let filename = format!("{}{}.epub", link.split("/").nth(2).unwrap(), today.format("%e%b%y"));
...
mod tests {
    ...
    async fn test_parse_ward_toc() {
        let chapters = get_chapter_list("https://parahumans.net/")
            .await
            .unwrap();
        dbg!(&chapters);
    ...
    async fn test_parse_worm_toc() {
        let chapters = get_chapter_list("http://parahumans.wordpress.com/table-of-contents/")
            .await
            .unwrap();
